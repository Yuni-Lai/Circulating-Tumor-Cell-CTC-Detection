...
inputPath='/dataserver145/image/Yuni/CTC/01.1_patients_samples/01_batch/01_data/'
outputPath='/dataserver145/image/Yuni/CTC/01.1_patients_samples/01_batch/'
...
for imageP in imagePAll:
    ...
    for files in imagelist:
        if contain_zh(files):
            #print('%s contain Chinese words' %files)
            continue
        if fnmatch.fnmatch(files, '*_BF.tif'):
            #leng=leng+1
            name_num=files.rsplit("_",1)[0].rsplit('/',1)[1]
            imagePath=files.rsplit('/',1)[0]
