...
# white_blood
# inputPath='/dataserver145/image/Yuni/CTC/01.2_cellline_samples/01_50tumor_50whiteBlood/01_data/20200421-WBC-01/'
# outputPath='/dataserver145/image/Yuni/CTC/01.2_cellline_samples/01_50tumor_50whiteBlood/02.1_whiteBlood_samples/'

#50WBC+50CMF
inputPath='/dataserver145/image/Yuni/CTC/01.2_cellline_samples/01_50tumor_50whiteBlood/01_data/20200421-WBC+CMF-7-02/'
outputPath='/dataserver145/image/Yuni/CTC/01.2_cellline_samples/01_50tumor_50whiteBlood/02.2_50CMF+50whiteBlood_samples/'


if not os.path.exists(outputPath+'02_segmented/'):
    os.makedirs(outputPath+'02_segmented/')#this is for the pkl result that contains all interest results such as size of cell, nuclues, combined images...; and also the intuitive cell segmented results, blue for nuclues, and green for cells.
    
if not os.path.exists(outputPath+'03_CTC_result_images/'):
    os.makedirs(outputPath+'03_CTC_result_images/')# this is for intuitive result of CTC estimate accodring to p value. different color means different significant level of P value.


#this is the data of tumor patients sample------    
...
#            CTC_mask_extreme=CTC_mask_extreme+temp   
    ...
    savefile=outputPath+'03_CTC_result_images/'+name+"_seg_CTC.jpg"
...
matplotlib.use('Agg')
#--preprocessing the BF(cell) and DAPI(nulcleus) images-----
...
#---segmented cell according to the gradient---------
...
#---segment cell angain according to voronoi method based on the rule: one nucleus one cell
...
'''
#---segement cell according to the subtraction BF and the average of BF image. average is the background, cell is higher/lower than then background.
...
#this function is wroten to smooth the cell. but it is not necessary
...
#---check the level of overlap between cell can nucleus---
...
#--adjust the imgage until the maximum overlap-----
...
#---this is the whole workflow of cell&nucleus detection and CTC estimation-------
def cell_detection(inputt):
    ...
    f = open(f"{outputPath}02_segmented/{n}.{num}.result.pkl", 'wb')
    ...
    savefile=outputPath+'02_segmented/'+name+"_seg_image.jpg"
    plt.savefig(savefile,dpi=600)
    #plt.close()
    return(Blue_dia,Cell_dia,Yel_int,Blue_int)


#=====this is for the website showing================================
...
#-----load the data here---------------------------
# subfolder=os.listdir(inputPath)
# imagePAll=[]
# for sub in subfolder:
#     imagePAll.append(inputPath+sub)
imagePAll=os.listdir(inputPath)
...
def contain_zh(word):
    ...
    '''
    #word='/dataserver145/image/Yuni/CTC/01.1_patients_samples/02_batch/01_data/20200404/20200404-20B00100048-复查/9_DAPI.tif'
    word_de = word.encode('utf-8').decode('utf-8')
    #global zh_pattern
    match = zh_pattern.search(word_de)
    if match:
        dic=['复查','分型','个']
        for d in dic:
            #d='复查'
...
n=inputPath.split('/')[-2]
#imagelist =get_filelist(imageP)
#os.walk(imageP)

for files in imagePAll:
    if fnmatch.fnmatch(files, '*_BF.tif'):
        #leng=leng+1
        name_num=files.rsplit("_",1)[0]
        imagePath=inputPath
        list_process.append([name_num,imagePath])
        print(name_num,"of ",imagePath)


#cell_detection(list_process[0])#please test it before running---


from tqdm import tqdm
import concurrent.futures
import time
#--multithreading function---general-----------
